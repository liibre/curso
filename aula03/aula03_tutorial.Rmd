...
(Nada deve aparecer, se algo aparecer avisa o/)
...
ex. `git config user.name "Andrea Sánchez-Tapia"`
...
1. `Preferências > git/svn`
1. Checar que git está apontando para um arquivo `git.exe` em windows, em mac e linux `/usr/bin/git`
3. Se você nunca fez isto, não deve ter nada no campo da chave RSA
4. Clique `Criar RSA Key`
5. Ver a RSA Key, copiar a chave. Ela é uma chave que identifica seu computador e vamos copiá-la em GitHub.com
...
__Vamos adicionar um remoto criado no GitHub __
